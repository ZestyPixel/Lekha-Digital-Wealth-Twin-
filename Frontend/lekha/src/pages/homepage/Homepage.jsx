...
export default function HomePage() {
  ...
  useEffect(() => {
    //To get all the required data for the homepage.
  ...
  return (
    <div>
      <div className="card-container">
        ...
        <Link to={"/networth"} state={protectedData?.asset ?? []}>
          <PieChartCard Data={protectedData?.asset} />
        </Link>
        <Link to={"/wealth"}>
          <WealthCard Advice={advice} />
        </Link>
        <Link
          to={"/recenttransactions"}
          state={protectedData?.transaction ?? []}
        >
          {" "}
          <LastTransaction
            data={
              protectedData?.transaction?.[protectedData.transaction.length - 1]
            }
          />{" "}
        </Link>{" "}
        ...
        <Link to={"/monthlyexpenses"}>
          <MonthlyExpensesCard Data={protectedData?.transaction} />
        </Link>
        <BudgetCard
          DataP={protectedData?.profile}
          DataT={protectedData?.transaction}
        />
      </div>
